...
jsmpeg.prototype = {
	...
	socketBufferSize: 512 * 1024, // 512kb each
	...
	decodeSocketHeader: function (data) {
	...
	recordedFrames: 0,
	recordedSize: 0,
	didStartRecordingCallback: null,
	...
	recordFrameFromCurrentBuffer: function () {
		if (!this.isRecording) {
			return;
		}
	...
	seekToFrame: function (seekFrame, seekExact) {
	...
	seekToTime: function (time, seekExact) {
	...
	macroblockType: 0,
	macroblockIntra: false,
	macroblockMotFw: false,
	...
	copyBlockToDestination: function (blockData, destArray, destIndex, scan) {
	...
	addBlockToDestination: function (blockData, destArray, destIndex, scan) {
		for (var n = 0; n < 64; n += 8, destIndex += scan + 8) {
			destArray[destIndex + 0] += blockData[n + 0];
			destArray[destIndex + 1] += blockData[n + 1];
			destArray[destIndex + 2] += blockData[n + 2];
	...
	copyValueToDestination: function (value, destArray, destIndex, scan) {
		for (var n = 0; n < 64; n += 8, destIndex += scan + 8) {
	...
	addValueToDestination: function (value, destArray, destIndex, scan) {
		for (var n = 0; n < 64; n += 8, destIndex += scan + 8) {
			destArray[destIndex + 0] += value;
			destArray[destIndex + 1] += value;
			destArray[destIndex + 2] += value;
	...
	copyBlockToDestinationClamp: function (blockData, destArray, destIndex, scan) {
		var n = 0;
		for (var i = 0; i < 8; i++) {
			for (var j = 0; j < 8; j++) {
				var p = blockData[n++];
	...
	addBlockToDestinationClamp: function (blockData, destArray, destIndex, scan) {
		var n = 0;
		for (var i = 0; i < 8; i++) {
			for (var j = 0; j < 8; j++) {
